 ...
 * 3. Access this index.php via your web browser
 */
...
if (preg_match('/^\/api\/user/', $path)) {
    ...
    // Homepage - Show app status and info
    ?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .container { max-width: 700px; text-align: center; color: white; }
        ...
        .status h2 { text-align: center; margin-bottom: 1rem; font-size: 1.3rem; }
        .status-item {
            display: flex;
        ...
        .api-docs h2 { margin-bottom: 1rem; text-align: center; }
        .endpoint {
...
<body>
    <div class="container">
        <h1>🔗 Linkon</h1>
        <p class="tagline">Securely store data online, create archives, and share content with publicly accessible short links.</p>
        ...
        <div class="status">
            ...
            echo '<div class="status-item"><span>PHP Version</span><span class="' . ($phpOk ? 'status-ok' : 'status-error') . '">' . PHP_VERSION . ($phpOk ? ' ✓' : ' ✗ (8.0+ required)') . '</span></div>';
            
            // Check PDO extension
            $pdoOk = extension_loaded('pdo');
            echo '<div class="status-item"><span>PDO Extension</span><span class="' . ($pdoOk ? 'status-ok' : 'status-error') . '">' . ($pdoOk ? 'Loaded ✓' : 'Missing ✗') . '</span></div>';
            
            // Check OpenSSL extension
            $opensslOk = extension_loaded('openssl');
            echo '<div class="status-item"><span>OpenSSL Extension</span><span class="' . ($opensslOk ? 'status-ok' : 'status-error') . '">' . ($opensslOk ? 'Loaded ✓' : 'Missing ✗') . '</span></div>';
            
            // Check config file
            $configExists = file_exists(__DIR__ . '/config/config.php');
            echo '<div class="status-item"><span>Configuration</span><span class="' . ($configExists ? 'status-ok' : 'status-error') . '">' . ($configExists ? 'Found ✓' : 'Missing ✗ (edit config/config.php)') . '</span></div>';
            
            // Check database connection
            $dbOk = false;
            $dbError = '';
            if ($configExists && $pdoOk) {
                try {
            ...
            echo '<div class="status-item"><span>Database</span><span class="' . ($dbOk ? 'status-ok' : 'status-error') . '">' . ($dbOk ? 'Connected ✓' : 'Not connected ✗') . '</span></div>';
            
            $allOk = $phpOk && $pdoOk && $opensslOk && $configExists && $dbOk;
            ?>
            <div class="status-item" style="margin-top: 1rem; border-top: 2px solid rgba(255,255,255,0.2); padding-top: 1rem;">
                <span><strong>Overall Status</strong></span>
                <span class="<?php echo $allOk ? 'status-ok' : 'status-error'; ?>">
                    <strong><?php echo $allOk ? '✓ Ready to use!' : '✗ Setup required'; ?></strong>
                </span>
            </div>
        </div>
